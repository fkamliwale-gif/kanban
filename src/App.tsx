...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {page === "team" && <TeamPage members={members} tasks={tasks} addMember={addMember} updateMember={updateMember} deleteMember={deleteMember} />}
...
function TeamPage({members,tasks,addMember,updateMember,deleteMember}:{members:TeamMember[];tasks:Task[];addMember:(x:Omit<TeamMember,"id">)=>void;updateMember:(id:string,x:Omit<TeamMember,"id">)=>void;deleteMember:(id:string)=>void}) { const [modal,setModal]=useState<"create"|TeamMember|null>(null);return <><PageTitle title="Team" subtitle="Manage project team members." action={<button className="primary-btn small" onClick={()=>setModal("create")}>+ Add Member</button>}/><div className="team-grid">{members.map(m=>{const assigned=tasks.filter(t=>t.assignedTo===m.id);return <div className="member-card" key={m.id}><div className="member-avatar">{m.name.charAt(0)}</div><h3>{m.name}</h3><StatusBadge value={m.role}/><p>{m.email}</p><div className="member-stats"><span><b>{assigned.length}</b>Assigned</span><span><b>{assigned.filter(t=>t.status==="Completed").length}</b>Completed</span><span><b>{assigned.filter(t=>t.status!=="Completed").length}</b>Active</span></div><div className="card-actions"><button onClick={()=>setModal(m)}>Edit</button><button className="danger-text" onClick={()=>window.confirm(`Delete ${m.name}?`)&&deleteMember(m.id)}>Delete</button></div></div>})}</div>{members.length===0&&<Empty text="No team members yet."/>}{modal&&<MemberModal initial={modal === "create" ? null : modal} onClose={()=>setModal(null)} onSave={data=>{modal === "create"?addMember(data):updateMember((modal as TeamMember).id,data);setModal(null)}}/>}</>; }
