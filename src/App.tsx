...
type ProjectStatus = "Planning" | "Not Started" | "In Progress" | "On Hold" | "Completed" | "Cancelled";
...
interface Activity { id: string; action: string; user: string; createdAt: string; }

const PROJECT_STATUSES: ProjectStatus[] = ["Planning", "Not Started", "In Progress", "On Hold", "Completed", "Cancelled"];
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {page === "dashboard" && <Dashboard projects={projects} tasks={tasks} members={members} activities={activities} openProject={openProject} />}
      {page === "projects" && <Projects projects={projects} tasks={tasks} members={members} user={user} createProject={createProject} updateProject={updateProject} deleteProject={deleteProject} openProject={openProject} />}
      {page === "projectDetails" && selectedProjectId && <ProjectDetails projectId={selectedProjectId} projects={projects} tasks={tasks} members={members} updateTask={updateTask} changeTaskStatus={changeTaskStatus} go={go} />}
      {page === "tasks" && <TasksPage projects={projects} tasks={tasks} members={members} createTask={createTask} updateTask={updateTask} deleteTask={deleteTask} changeTaskStatus={changeTaskStatus} />}
      {page === "calendar" && <CalendarPage tasks={tasks} projects={projects} members={members} />}
      {page === "team" && <TeamPage user={user} members={members} tasks={tasks} addMember={addMember} updateMember={updateMember} deleteMember={deleteMember} />}
      {page === "reports" && <Reports projects={projects} tasks={tasks} />}
      {page === "activity" && <ActivityPage activities={activities} />}
      {page === "settings" && <Settings user={user} logout={logout} />}
    </main>
  </div>;
}
...
function Sidebar({ page, go, logout }: { page: Page; go: (p: Page) => void; logout: () => void }) {
  const items: [Page, string, string][] = [["dashboard", "▣", "Dashboard"], ["projects", "▤", "Projects"], ["tasks", "✓", "Tasks"], ["calendar", "◫", "Calendar"], ["team", "♟", "Team"], ["reports", "▥", "Reports"], ["activity", "◷", "Activity"], ["settings", "⚙", "Settings"]];
  return <aside className="sidebar"><div className="sidebar-brand"><span>✓</span><b>Task<span>Flow</span></b></div><nav>{items.map(([p, icon, label]) => <button key={p} className={page === p || (page === "projectDetails" && p === "projects") ? "active" : ""} onClick={() => go(p)}><strong>{icon}</strong>{label}</button>)}</nav><button className="logout-side" onClick={logout}>↪ Logout</button></aside>;
}
function Header({ user }: { user: User }) { return <header className="topbar"><div><span className="mobile-title">TaskFlow</span></div><div className="top-user"><button className="notification">♢</button><div className="avatar">{user.name.charAt(0).toUpperCase()}</div><div><b>{user.name}</b><small>{user.role}</small></div></div></header>; }

function PageTitle({ title, subtitle, action }: { title: string; subtitle?: string; action?: React.ReactNode }) { return <div className="page-title"><div><h1>{title}</h1>{subtitle && <p>{subtitle}</p>}</div>{action}</div>; }
function StatCard({ label, value, icon }: { label: string; value: number; icon: string }) { return <div className="stat-card"><span className="stat-icon">{icon}</span><div><p>{label}</p><h2>{value}</h2></div></div>; }
function StatusBadge({ value }: { value: string }) { return <span className={`badge ${value.toLowerCase().replaceAll(" ", "-")}`}>{value}</span>; }
function PriorityBadge({ value }: { value: Priority }) { return <span className={`priority ${value.toLowerCase()}`}>{value}</span>; }

function projectProgress(projectId: string, tasks: Task[]) { const list = tasks.filter(t => t.projectId === projectId); return list.length ? Math.round(list.filter(t => t.status === "Completed").length / list.length * 100) : 0; }
function Dashboard({ projects, tasks, members, activities, openProject }: { projects: Project[]; tasks: Task[]; members: TeamMember[]; activities: Activity[]; openProject: (id: string) => void }) {
  const active = projects.filter(p => p.status === "In Progress").length; const completed = projects.filter(p => p.status === "Completed").length; const overdue = tasks.filter(isOverdue).length;
  return <><PageTitle title="Dashboard" subtitle={`Welcome back. Today is ${formatDate(today())}.`} /><div className="stats-grid"><StatCard label="Total Projects" value={projects.length} icon="▤"/><StatCard label="Active Projects" value={active} icon="◉"/><StatCard label="Completed Projects" value={completed} icon="✓"/><StatCard label="Total Tasks" value={tasks.length} icon="☷"/><StatCard label="To Do" value={tasks.filter(t=>t.status==="To Do").length} icon="○"/><StatCard label="In Progress" value={tasks.filter(t=>t.status==="In Progress").length} icon="◐"/><StatCard label="Completed Tasks" value={tasks.filter(t=>t.status==="Completed").length} icon="✓"/><StatCard label="Overdue Tasks" value={overdue} icon="!"/></div><div className="dashboard-grid"><section className="panel"><div className="panel-heading"><h2>Project Progress</h2><span>{projects.length} projects</span></div>{projects.length === 0 ? <Empty text="No projects yet."/> : <div className="progress-list">{projects.map(p => <button className="progress-row" key={p.id} onClick={() => openProject(p.id)}><div className="row-main"><div><b>{p.name}</b><small>{members.find(m=>m.id===p.managerId)?.name || "No manager"} · {p.status}</small></div><strong>{projectProgress(p.id,tasks)}%</strong></div><div className="progress-track"><span style={{width: `${projectProgress(p.id,tasks)}%`}}/></div><div className="row-meta"><span>{tasks.filter(t=>t.projectId===p.id && t.status==="Completed").length}/{tasks.filter(t=>t.projectId===p.id).length} tasks completed</span><span>Due {formatDate(p.dueDate)}</span></div></button>)}</div>}</section><section className="panel"><div className="panel-heading"><h2>Recent Activity</h2><span>{activities.length}</span></div>{activities.length === 0 ? <Empty text="No activity yet. Create a project or task to get started."/> : <ActivityList activities={activities.slice(0,7)}/>}</section></div></>;
}
function Empty({ text }: { text: string }) { return <div className="empty">{text}</div>; }
function ActivityList({ activities }: { activities: Activity[] }) { return <div className="activity-list">{activities.map(a => <div className="activity-item" key={a.id}><span className="activity-dot">•</span><div><b>{a.action}</b><small>{a.user} · {formatDateTime(a.createdAt)}</small></div></div>)}</div>; }
...
function TaskModal({initial,projects,members,onClose,onSave}:{initial:Task|null;projects:Project[];members:TeamMember[];onClose:()=>void;onSave:(x:Omit<Task,"id"|"createdAt">)=>void}) { const [title,setTitle]=useState(initial?.title||"");const [description,setDescription]=useState(initial?.description||"");const [projectId,setProjectId]=useState(initial?.projectId||projects[0]?.id||"");const [assignedTo,setAssignedTo]=useState(initial?.assignedTo||members[0]?.id||"");const [priority,setPriority]=useState<Priority>(initial?.priority||"Medium");const [status,setStatus]=useState<TaskStatus>(initial?.status||"To Do");const [startDate,setStartDate]=useState(initial?.startDate||today());const [dueDate,setDueDate]=useState(initial?.dueDate||"");const submit=(e:FormEvent)=>{e.preventDefault();if(!title.trim()||!projectId||!dueDate)return;onSave({title,description,projectId,assignedTo,priority,status,startDate,dueDate});};return <Modal title={initial?"Edit Task":"Create Task"} onClose={onClose}><form onSubmit={submit}><div className="form-grid"><Field label="Task title"><input value={title} onChange={e=>setTitle(e.target.value)} required/></Field><Field label="Project"><select value={projectId} onChange={e=>setProjectId(e.target.value)}>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="Assigned member"><select value={assignedTo} onChange={e=>setAssignedTo(e.target.value)}><option value="">Unassigned</option>{members.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select></Field><Field label="Priority"><select value={priority} onChange={e=>setPriority(e.target.value as Priority)}>{PRIORITIES.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Status"><select value={status} onChange={e=>setStatus(e.target.value as TaskStatus)}>{TASK_STATUSES.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Start date"><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></Field><Field label="Due date"><input type="date" value={dueDate} onChange={e=>setDueDate(e.target.value)} required/></Field><div className="full-field"><Field label="Description"><textarea value={description} onChange={e=>setDescription(e.target.value)} rows={3}/></Field></div></div><div className="modal-actions"><button type="button" onClick={onClose}>Cancel</button><button className="primary-btn" type="submit">{initial?"Save Changes":"Create Task"}</button></div></form></Modal>; }

function CalendarPage({tasks,projects,members}:{tasks:Task[];projects:Project[];members:TeamMember[]}) { const [month,setMonth]=useState(new Date()); const year=month.getFullYear();const m=month.getMonth();const days=new Date(year,m+1,0).getDate();const first=new Date(year,m,1).getDay();const cells=Array.from({length:first+days},(_,i)=>i<first?null:i-first+1);const tasksByDay=(d:number)=>tasks.filter(t=>{if(!t.dueDate)return false;const x=new Date(`${t.dueDate}T00:00:00`);return x.getFullYear()===year&&x.getMonth()===m&&x.getDate()===d;});return <><PageTitle title="Calendar" subtitle="View tasks by their due dates."/><section className="panel calendar-panel"><div className="calendar-head"><button onClick={()=>setMonth(new Date(year,m-1,1))}>←</button><h2>{month.toLocaleDateString("en-IN",{month:"long",year:"numeric"})}</h2><button onClick={()=>setMonth(new Date(year,m+1,1))}>→</button></div><div className="calendar-grid weekday">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><b key={d}>{d}</b>)}</div><div className="calendar-grid">{cells.map((d,i)=><div className="calendar-day" key={i}>{d&&<><strong>{d}</strong>{tasksByDay(d).map(t=><div className={`calendar-task ${isOverdue(t)?"calendar-overdue":""}`} key={t.id} title={t.title}>{t.title}<small>{projects.find(p=>p.id===t.projectId)?.name||""} · {members.find(m=>m.id===t.assignedTo)?.name||"-"}</small></div>)}</>}</div>)}</div></section></>; }

function TeamPage({user,members,tasks,addMember,updateMember,deleteMember}:{user:User;members:TeamMember[];tasks:Task[];addMember:(x:Omit<TeamMember,"id">)=>void;updateMember:(id:string,x:Omit<TeamMember,"id">)=>void;deleteMember:(id:string)=>void}) { const [modal,setModal]=useState<"create"|TeamMember|null>(null); const isAdmin=user.role==="Admin"; return <><PageTitle title="Team" subtitle={isAdmin ? "Manage project team members." : "View your project team and assigned workload."} action={isAdmin ? <button className="primary-btn small" onClick={()=>setModal("create")}>+ Add Member</button> : undefined}/>{!isAdmin&&<div className="permission-note">🔒 Member management is restricted to administrators.</div>}<div className="team-grid">{members.map(m=>{const assigned=tasks.filter(t=>t.assignedTo===m.id);return <div className="member-card" key={m.id}><div className="member-avatar">{m.name.charAt(0)}</div><h3>{m.name}</h3><StatusBadge value={m.role}/><p>{m.email}</p><div className="member-stats"><span><b>{assigned.length}</b>Assigned</span><span><b>{assigned.filter(t=>t.status==="Completed").length}</b>Completed</span><span><b>{assigned.filter(t=>t.status!=="Completed").length}</b>Active</span></div>{isAdmin&&<div className="card-actions"><button onClick={()=>setModal(m)}>Edit</button><button className="danger-text" onClick={()=>window.confirm(`Delete ${m.name}?`)&&deleteMember(m.id)}>Delete</button></div>}</div>})}</div>{members.length===0&&<Empty text="No team members yet."/>}{isAdmin&&modal&&<MemberModal initial={modal === "create" ? null : modal} onClose={()=>setModal(null)} onSave={data=>{modal === "create"?addMember(data):updateMember((modal as TeamMember).id,data);setModal(null)}}/>}</>; }
function MemberModal({initial,onClose,onSave}:{initial:TeamMember|null;onClose:()=>void;onSave:(x:Omit<TeamMember,"id">)=>void}){const[name,setName]=useState(initial?.name||"");const[email,setEmail]=useState(initial?.email||"");const[role,setRole]=useState<Role>(initial?.role||"Member");return <Modal title={initial?"Edit Member":"Add Member"} onClose={onClose}><form onSubmit={e=>{e.preventDefault();if(name&&email)onSave({name,email,role})}}><Field label="Name"><input value={name} onChange={e=>setName(e.target.value)} required/></Field><Field label="Email"><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></Field><Field label="Role"><select value={role} onChange={e=>setRole(e.target.value as Role)}>{ROLES.map(r=><option key={r}>{r}</option>)}</select></Field><div className="modal-actions"><button type="button" onClick={onClose}>Cancel</button><button className="primary-btn" type="submit">Save</button></div></form></Modal>}
